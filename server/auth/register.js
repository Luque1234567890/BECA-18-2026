import { api, assertSameOrigin, clean, cookie, hashPassword, json, normalEmail, phone, readJson, recordLegalAcceptance, requireMethod, signSession } from '../_lib.js';
export default api(async request => {
  ...
  const termsAccepted = body.termsAccepted === true || body.termsAccepted === 'on';
  ...
  if (!termsAccepted) return json({ error: 'Debes aceptar los Términos y Condiciones y la Política de Privacidad.' }, 422);
  ...
  try {
    ...
    await recordLegalAcceptance(client, user.rows[0].id);
