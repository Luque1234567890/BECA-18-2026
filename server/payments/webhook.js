...
function verifySignature(request, paymentId) { const secret = process.env.MERCADOPAGO_WEBHOOK_SECRET; if (!secret) return false; const signature = request.headers.get('x-signature') || ''; const requestId = request.headers.get('x-request-id') || ''; const parts = Object.fromEntries(signature.split(',').map(item => item.trim().split('='))); const manifest = [`id:${paymentId || ''};`, requestId && `request-id:${requestId};`, `ts:${parts.ts || ''};`].filter(Boolean).join(''); const expected = crypto.createHmac('sha256', secret).update(manifest).digest('hex'); const received = Buffer.from(parts.v1 || '', 'hex'); const calculated = Buffer.from(expected, 'hex'); return received.length === calculated.length && received.length > 0 && crypto.timingSafeEqual(received, calculated); }
